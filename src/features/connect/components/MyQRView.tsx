...
export const MyQRView = React.memo(() => {
  ...
  const qrValue = useMemo(() => {
    ...
    if (shareContact) {
      payload.contact = {
        email: userData?.STUDENT_REGISTRATION_DETAIL_sStdEmail || userData?.std_adm_email || "",
        mobileNumber: userData?.STUDENT_REGISTRATION_DETAIL_sStdMobile || userData?.std_adm_mobile || "",
      };
    }
    ...
    studentId,
    loginData?.student_name, loginData?.college_name, loginData?.batch_name, loginData?.sem_no,
    userData?.profile_pict_cur_url,
    userData?.STUDENT_REGISTRATION_DETAIL_sStdEmail, userData?.std_adm_email,
    userData?.STUDENT_REGISTRATION_DETAIL_sStdMobile, userData?.std_adm_mobile,
    shareCollege, shareContact, shareSocial, shareProfilePic,
    social,
  ]);
  ...
  return (
    ...
    >
      ...
      <View className="bg-surface dark:bg-surface rounded-xl p-4 mb-6 border border-border">
        <Text className="text-lg font-semibold text-ink-900 dark:text-white mb-4 font-display">Share Settings</Text>
        
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-ink-600 dark:text-ink-300 font-medium">Share Profile Picture</Text>
          <Switch value={shareProfilePic} onValueChange={setShareProfilePic} />
        </View>

        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-ink-600 dark:text-ink-300 font-medium">Share College Info</Text>
          <Switch value={shareCollege} onValueChange={setShareCollege} />
        </View>
        
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-ink-600 dark:text-ink-300 font-medium">Share Contact Info</Text>
          <Switch value={shareContact} onValueChange={setShareContact} />
        </View>
        
        <View className="flex-row justify-between items-center">
          <Text className="text-ink-600 dark:text-ink-300 font-medium">Share Social Links</Text>
      ...
      <View className="bg-surface dark:bg-surface rounded-xl p-4 mb-8 border border-border">
        <Text className="text-lg font-semibold text-ink-900 dark:text-white mb-4 font-display">Social Profiles</Text>
