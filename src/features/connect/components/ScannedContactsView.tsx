        ...
        <View className="flex-row items-center flex-1">
          {profilePicUrl ? (
            ...
            <View className="w-12 h-12 bg-cobalt-500/10 rounded-full items-center justify-center mr-4">
              <Text className="text-cobalt-500 font-bold text-lg">{getInitials(fullName)}</Text>
            </View>
          )}
          <View className="flex-1">
            ...
            {college?.name && (
              <Text className="text-sm text-ink-500 dark:text-ink-400 font-sans" numberOfLines={1}>
                {college.name}
              </Text>
            )}
    ...
    <View className="flex-1 p-4 bg-base">
      {scannedContacts.length === 0 ? (
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="text-lg font-medium text-ink-900 dark:text-white font-display">No contacts yet</Text>
