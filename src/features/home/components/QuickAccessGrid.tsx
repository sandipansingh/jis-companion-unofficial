...
export interface MenuItem {
  ...
  icon: LucideIcon;
  color: string;
  /** Optional short description — used by desktop action cards, ignored on mobile. */
  description?: string;
}
...
export function QuickAccessGrid({ items, onItemPress }: QuickAccessGridProps) {
  ...
  return (
    <View className="mt-4">
      <Text className="text-base text-ink-900 dark:text-ink-100 mb-4 font-display">
        Quick Access
      </Text>
          ...
          >
            <View
              ...
              style={{
                ...
                shadowOpacity: device.isAndroid ? 0 : 0.14,
                shadowRadius: 8,
                elevation: device.isAndroid ? 0 : 3,
              }}
            >
              <item.icon size={24} color={item.color} strokeWidth={1.8} />
            </View>
            <Text className="text-[11px] text-ink-600 dark:text-ink-400 text-center leading-4 font-sans-md">
              {item.title}
            </Text>
          </TouchableOpacity>
        ))}
