...
export function HeaderCard({
  ...
  if (variant === 'hero') {
    return (
      <View className="rounded-3xl p-6 mb-6 overflow-hidden bg-cobalt-600 dark:bg-cobalt-900">
        <View
        ...
        <Text className="font-sans-bold text-white text-[22px] mb-1.5">{title}</Text>
        ...
        {stats && stats.length > 0 && (
          <View className="flex-row gap-2">
            {stats.map((stat) => (
              ...
              >
                <Text className="font-sans-bold text-white text-lg">{stat.value}</Text>
                <Text
  ...
  return (
    ...
    >
      ...
      >
        <Icon size={iconSize} color={isDark ? colors.text : colors.cobalt[600]} />
      </View>
